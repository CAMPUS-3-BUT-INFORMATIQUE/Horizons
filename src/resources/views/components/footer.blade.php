<div>
    <footer class="bg-neutral-700 text-center lg:text-left absolute bottom-0 w-screen">
        <div class="p-4 text-center text-surface text-white">
            <div class="flex flex-col md:flex-row justify-evenly items-center">
                <div class="-ml-7 md:w-40 flex flex-row items-center">
                    <img class="block max-w-52" src="{{ asset('images/couleur.png') }}" alt="Logo Horizons" />
                    <img class="max-md:max-w-44 max-md:h-fit md:hidden" src="{{ asset('images/logo_unicaen_blanc.png') }}" alt="Logo unicaen" />
                </div>
                <div class="text-center sm:text-sm" >
                    <div class="flex flex-col md:flex-row md:justify-evenly">
                        <div>
                            <a class="link" href="https://ecampus.unicaen.fr/local/staticpage/view.php?page=mentions_legales">
                                Mentions légales
                            </a>
                        </div>
                        <div class="mx-5"> · </div>
                        <div>
                            <a class="link" href="https://ecampus.unicaen.fr/local/staticpage/view.php?page=rgpd">
                                Vie privée
                            </a>
                        </div>
                        <div class="mx-5"> · </div>
                        <div>
                            <a class="link" href="https://ecampus.unicaen.fr/local/staticpage/view.php?page=charte_utilisation">
                                Charte utilisation
                            </a>
                        </div>
                        <div class="mx-5"> · </div>
                        <div>
                            <a class="link" href="https://www.unicaen.fr/">
                                Unicaen
                            </a>
                        </div>
                        <div class="mx-5"> · </div>
                        <div>
                            <a class="link" href="https://pedagogie.unicaen.fr/">
                                CEMU
                            </a>
                        </div>
                        <div class="mx-5"> · </div>
                        <div>
                            <a class="link" href="https://moodle.org/">
                                Moodle
                            </a>
                        </div>
                    </div>
                    <br>
                    <div>© 2024, all rights reserved</div>
                </div>
                <div class="w-40">
                    <img class="hidden md:block" src="{{ asset('images/logo_unicaen_blanc.png') }}" alt="Logo Unicaen" />
                </div>
            </div>
        </div>
    </footer>
</div>
